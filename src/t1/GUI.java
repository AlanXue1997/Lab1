...
class DrawPanel extends JPanel{
	...
	public static final int ratio = 60;           //200   130
	...
	private void initialize() {
			...
			public void actionPerformed(ActionEvent e) {
				JFileChooser jfc = new JFileChooser();
			     ...
			     if(type.equals(".txt")){
			    	 ...
			    	 textField_5.setText("Error: this file type is not .txt!");
		...
		btnNewButton_4.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(now_step > walks[0]){
					walks = G.randomWalk();
					 writename = new File("/Users/jethro/Documents/random.txt");
			    	  try {
						writename.createNewFile();
						BufferedWriter out = new BufferedWriter(new FileWriter(writename)); 
						for(int k=1;k<=walks[0];k++){
							if(g.points[walks[k]].st!=null){
								out.write(g.points[walks[k]].st+" ");
							}
						}
						out.write("\n");
						out.flush();
					...
					now_step = 1;
					flag = false;
					for(int i=0;i<g.point_state.length;i++){
						g.point_state[i] = 0;
					}
					for(int i=0;i<g.points.length;i++){
						g.point_state[i] = 0;
				...
				while(g.points[walks[now_step]].st==null){
					if(now_step!=1){
						g.edge_state[walks[now_step-1]][walks[now_step]] =1;
					}
					now_step += 1;
					if(now_step > walks[0]){
						walks = G.randomWalk();
				...
				g.point_state[walks[now_step]] = 1;
				if(now_step!=1){
					g.edge_state[walks[now_step-1]][walks[now_step]] = 1;
				}
				flag = true;
	...
	public int[] randomWalk(){
		...
		while(out_degree[array[array[0]]] > 0 || array[array[0]] >= N){
			if(array[array[0]] < N){
				...
				for(int i = 0; i < dummy; ++i){
					if(internal_map[array[array[0]]][i] == 1 && father != i){
						array[++array[0]] = i;
						break;
					}
				}
